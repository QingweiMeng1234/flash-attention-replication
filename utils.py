...
def beam_search_decode(model, src, src_mask, max_len, start_symbol, beam_size, end_idx):
    ...
    for i in range(max_len - 1):
        ...
        # Multiply current scores with new probabilities (log space addition)
        
        scores = prob + scores.view(-1,1)
        
        scores,score_idx = scores.view(-1).topk(beam_size)
        beam_idx = score_idx // vocab_size
        token_idx = score_idx % vocab_size
        

        # Form the new ys sequences
    ...
    # Return the best sequence after the loop
    top_score_idx = scores.argmax()
    ys = ys[top_score_idx].unsqueeze(0)
